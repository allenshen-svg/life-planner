...
const app = createApp({
  setup() {
    ...
    async function savePlan() {
      ...
      try {
        if (editingPlan.value) {
          await fetch(`${API}/api/plans/${editingPlan.value}`, {
    ...
    async function loadDiary() {
      try {
        const r = await fetch(`${API}/api/diary`);
    ...
    async function saveDiary() {
      ...
      try {
        if (editingDiary.value) {
          await fetch(`${API}/api/diary/${editingDiary.value}`, {
            method: 'PUT', headers: {'Content-Type':'application/json'},
            body: JSON.stringify(f)
          });
        } else {
    ...
    async function deleteDiary(id) {
      ...
      await fetch(`${API}/api/diary/${id}`, { method: 'DELETE' });
      await loadDiary();
    }
    ...
    async function aiPost(url, body) {
      body.provider = aiProvider.value;
      body.model = aiProvider.value === 'deepseek' ? 'deepseek-chat' : aiProvider.value === 'zhipu' ? 'GLM-4-Flash' : 'Qwen/Qwen2.5-7B-Instruct';
    ...
    async function getHotActivities() {
      ...
      aiPlanResult.value = '加载中...';
      try {
        const r = await aiPost('/api/ai/hot-activities', { city: planCity.value });
        aiPlanResult.value = r.activities;
      } catch (e) { aiPlanResult.value = '获取失败，请重试'; }
    ...
    async function getSuggestPlaces(cat) {
      aiPlanLoading.value = true;
      aiPlanResult.value = '加载中...';
      const loc = filteredPlans.value[0]?.location || '';
      try {
        const r = await aiPost('/api/ai/suggest-places', { city: planCity.value, location: loc, category: cat });
        aiPlanResult.value = r.suggestions;
      } catch (e) { aiPlanResult.value = '获取失败，请重试'; }
      aiPlanLoading.value = false;
    }

    async function getTransport(from, to) {
      aiTransportLoading.value = true;
      aiTransportResult.value = '加载中...';
      try {
        const r = await aiPost('/api/ai/transport', { city: planCity.value, from, to });
        aiTransportResult.value = r.transport;
      } catch (e) { aiTransportResult.value = '获取失败'; }
      aiTransportLoading.value = false;
    }
    ...
    async function getLifeReview(period) {
      ...
      const periodPlans = plans.value.filter(p => p.date >= startStr && p.date <= endStr);
      const periodDiary = diaryEntries.value.filter(e => e.date >= startStr && e.date <= endStr);

      try {
        const r = await aiPost('/api/ai/life-review', { period, plans: periodPlans, entries: periodDiary });
        aiReviewResult.value = r.review;
      } catch (e) { aiReviewResult.value = '生成失败，请重试'; }
